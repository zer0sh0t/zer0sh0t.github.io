...
\left[\begin{array}{cc}
J_{H}^\top \\ J_{a}^\top
\end{array}\right]F_{GRF}
$$

where $$ D_i $$, $$ H_i $$, $$ B_i $$, and $$ J_i $$ are partitioned model parameters corresponding to the head 'H' and actuated 'a' joints. $$ F_{GRF} $$ denotes the ground reaction forces. $u$ embodies the joint actuation torques. The actuator parameters include: transmission inertia, internal damping, and dc motor constant. The ground contact model used in the simulator is given below:
